...
class InventorySummary(models.Model):
    ...
    def _get_move_line_data(
        self,
        data=[],
        set_ids=[],
        to_date=False,
        location_usage_internal=True,
        location_id=False,
    ):
        ...
        for quant in quants:
            ...
            if to_date:
                quant_lines = lines.filtered(
                    lambda l: l.date <= to_date
                    and l.product_id.id == quant.product_id.id
                    and (
                        l.location_id.id == quant.location_id.id
                        or l.location_dest_id.id == quant.location_id.id
                    )
                )
            else:
                quant_lines = lines.filtered(
                    lambda l: l.product_id.id == quant.product_id.id
                    and (
                        l.location_id.id == quant.location_id.id
                        or l.location_dest_id.id == quant.location_id.id
                    )
            ...
            in_lines = quant_lines.filtered(
                lambda l: l.location_dest_id == quant.location_id
            )
            out_lines = quant_lines.filtered(
                lambda l: l.location_id == quant.location_id
            )
            # Convert to base unit
            in_quantities = in_lines.mapped(
                lambda l: l.product_uom_id._compute_quantity(
                    l.qty_done, l.product_id.uom_id
                )
            )
            out_quantities = out_lines.mapped(
                lambda l: l.product_uom_id._compute_quantity(
                    l.qty_done, l.product_id.uom_id
                )
            )
            quantity = sum(in_quantities) - sum(out_quantities)
            standard_price = quant.product_id.with_company(
                quant.company_id
            ).standard_price
            ...
            data.append(
                {
                    ...
                    "last_out_date": max(out_lines.mapped("date"))
                    if out_lines
                    else False,
                    "quantity": quantity,
    ...
    def get_data(self, to_date=False, location_usage_internal=True, location_id=False):
        ...
        # Get stock move line data
        data, set_ids = self._get_move_line_data(
            data, set_ids, to_date, location_usage_internal, location_id
        )
