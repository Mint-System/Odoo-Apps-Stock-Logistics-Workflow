...
class StockBarcodeMRPController(StockBarcodeController):
    ...
    def try_open_mo(self, barcode):
        ...
        corresponding_mo = request.env['mrp.production'].search([('name', 'like', barcode)], limit=1)
        ...
        backorder_ids = corresponding_mo.procurement_group_id.mrp_production_ids.filtered(lambda o: o.state not in ['draft','done','cancel'])
    ...
    def try_open_wo(self, corresponding_mo):
        workorder_id = corresponding_mo.workorder_ids.filtered(lambda o: o.state in ['ready','progress'])
        if workorder_id:
            # _logger.warning(['workorder_id', workorder_id])
            if not workorder_id.is_user_working and workorder_id.working_state != 'blocked' and workorder_id.state in ('ready', 'progress', 'pending'):
